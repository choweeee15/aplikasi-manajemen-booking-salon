    ...
    <div class="content">
        <div class="row">
                            ...
                            <table class="table table-bordered table-hover">
                                <thead class="thead-dark text-center">
                                    <tr>
                                        ...
                                        <th>Status</th>
                                        <th>Bukti</th>
                                        <?php if (session()->get('level') == 1 || session()->get('level') == 10) : ?>
                                            <th>Aksi</th>
                                        <?php endif; ?>
                                    </tr>
                                </thead>
                                        ...
                                        foreach ($reservations as $reservation) : ?>
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <?php if ($reservation['status'] == 'Waiting for Fine Payment') : ?>
                                                            ...
                                                            <div class="modal-dialog">
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <form action="<?= base_url('ParkingController/bayarDenda/' . $reservation['id_reservasi']) ?>" method="post" enctype="multipart/form-data">
                                                                            <?= csrf_field() ?>
            ...
            <?= strtolower($reservation['metode_pembayaran_denda']) == 'ovo' ? 'bg-purple' : (strtolower($reservation['metode_pembayaran_denda']) == 'dana' ? 'bg-primary' : (strtolower($reservation['metode_pembayaran_denda']) == 'bank transfer' ? 'bg-dark' : (strtolower($reservation['metode_pembayaran_denda']) == 'gopay' ? 'bg-info' : 'bg-secondary'))) ?>"
                                                ...
                                                <td class="text-center">
                                                    <span class="badge 
                                                        <?= ($reservation['status'] == 'Terbayar') ? 'bg-warning' : (($reservation['status'] == 'Confirmed') ? 'bg-info' : 'bg-success') ?>"
                                                ...
                                                <?php if (session()->get('level') == 1 || session()->get('level') == 10) : ?>
                                                    <td class="text-center">
                                                        <?php if ($reservation['status'] == 'Terbayar') : ?>
                                                            <a href="<?= base_url('ParkingController/updateStatusReservasi/' . $reservation['id_reservasi']) ?>" class="btn btn-sm btn-warning">
                                                                <i class="fas fa-check"></i> Konfirmasi Pembayaran
                                                            </a>
                                                        <?php elseif ($reservation['status'] == 'Confirmed') : ?>
                                                            <a href="<?= base_url('ParkingController/updateStatusReservasi/' . $reservation['id_reservasi']) ?>" class="btn btn-sm btn-primary">
                                                                <i class="fas fa-door-open"></i> Checkin
                                                            </a>
                                                        <?php elseif ($reservation['status'] == 'Checked-in') : ?>
                                                            <a href="<?= base_url('ParkingController/updateStatusReservasi/' . $reservation['id_reservasi']) ?>" class="btn btn-sm btn-danger">
                                                                <i class="fas fa-door-closed"></i> Checkout
                                                            </a>
                                                        <?php elseif ($reservation['status'] == 'Waiting for Fine Payment') : ?>
                                    ...
                                    <?php else : ?>
                                        <tr>
                                            <td colspan="<?= (session()->get('level') == 1) ? '7' : ((session()->get('level') == 10) ? '5' : '6') ?>" class="text-center">
